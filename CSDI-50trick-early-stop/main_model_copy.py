...
class CSDI_base(nn.Module):
    ...
    def ddim_impute(self, observed_data, cond_mask, side_info, n_samples,ddim_eta=1,ddim_steps=10):
        ...
        for i in range(n_samples):
            ...
            print("ddim time steps sequence is :")
            print(ddim_timesteps_sequence)
            ...
            for step_number in range(ddim_timesteps - 1 , -1, -1):
                # t = torch.full((B, ), ddim_timesteps_sequence[step_number], dtype=torch.long).to(self.device)
                # previous_t = torch.full((B, ), ddim_timesteps_previous_sequence[step_number],dtype=torch.long).to(self.device)
                t = ddim_timesteps_sequence[step_number]
                previous_t =  ddim_timesteps_previous_sequence[step_number]

                # alpha_t = self.alpha[t]
                # alpha_previous_t = self.alpha[previous_t]
                alpha_t = torch.tensor(self.alpha[t]).to(self.device)
                alpha_previous_t = torch.tensor(self.alpha[previous_t]).to(self.device)
                ...
                pred_x0 = (current_sample - torch.sqrt(( 1 - alpha_t)) * predicted) / torch.sqrt(alpha_t)

                sigma_t = ddim_eta * torch.sqrt(
                    (1 - alpha_previous_t) / (1 - alpha_t) * (1 - alpha_t / alpha_previous_t)
                )

                pred_dir_xt = torch.sqrt(1 - alpha_previous_t - sigma_t ** 2) * predicted

                current_sample = torch.sqrt(alpha_previous_t) * pred_x0 + pred_dir_xt + sigma_t * torch.randn_like(current_sample)

            imputed_samples[:, i] = current_sample.detach()
        return imputed_samples


    def get_middle_impute_value(self, observed_data, cond_mask, side_info, n_samples):
        B, K, L = observed_data.shape

        imputed_samples = torch.zeros(B, n_samples, K, L).to(self.device)
        imputed_middle_samples = torch.zeros(B,n_samples,self.num_steps, K, L)
        ...
        for i in range(n_samples):
            ...
            for t in tqdm(range(self.num_steps - 1, -1, -1)):
                ...
                imputed_middle_samples[:, i, t] = current_sample.detach()
    ...
    def evaluate_middle_result(self, batch, n_samples):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        with torch.no_grad():
            ...
            samples,middle_samples = self.get_middle_impute_value(observed_data, cond_mask, side_info, n_samples)

            for i in range(len(cut_length)):  # to avoid double evaluation
                target_mask[i, ..., 0 : cut_length[i].item()] = 0
        return samples, middle_samples, observed_data, target_mask, observed_mask, observed_tp
