...
for time in range(0,args.times):
    ...
    if args.ddim:
        print("using ddim")
        ddim_ensemble(model, test_loader, nsample=args.nsample, scaler=1, name=(args.name + str(time) + ".pk"))
    else:
        ensemble(model, test_loader, nsample=args.nsample, scaler=1, name=(args.name + str(time) + ".pk"))
