...
for i, train_data_path in enumerate(train_data_path_list):
    ...
    # 由于此处是测试训练集上的结果，所以test_path直接使用train_data的path
    ...
    for file in os.listdir(foldername):
        if "model.pth" in file:
            epoch_number = file.split("-")[0]
            model.load_state_dict(torch.load(
                foldername + "/" + file
            ))
            print(f"begin to compute train error for file {file}")
            evaluate(model,test_loader1,test_loader2,nsample=1,foldername=foldername,epoch_number=epoch_number,name="train_error")
    '''
    if args.modelfolder == "":
        train(
            model,
